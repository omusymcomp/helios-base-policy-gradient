...
void flush_episode_if_needed(const rcsc::WorldModel &wm)
{
    ...
    if (episode_end && !episode_buffer.empty())
    {
        ...
        {
            csv_file << "match_id,ball_x,ball_y,player_x,player_y,player_vel_x,player_vel_y,cycle,action_index,original_reward,discounted_reward,field_eval_label,player_num";
            for (int i = 1; i <= 10; ++i)
            {
        ...
        {   
            ...
            csv_file << returns[t] << ",";
            csv_file << episode_buffer[t].field_eval_label << ",";
